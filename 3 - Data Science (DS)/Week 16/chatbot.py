...
class ImprovedHuggingFaceChatbot:
    def __init__(self, model_name: str = "microsoft/DialoGPT-medium", device: str = "gpu"):
        self.device = device
    ...
    def generate_response(self, input_text: str, max_length: int = 1000) -> str:
        try:
            ...
            new_user_input_ids = self.tokenizer.encode(
                input_text + self.tokenizer.eos_token, return_tensors='pt').to(self.device)
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Improved Hugging Face Chatbot")
    ...
    parser.add_argument("--device", type=str, default="cpu",
                        choices=["cpu", "cuda"], help="Device to use (default: cpu)")
